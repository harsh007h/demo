...
async function loadParties() {
    try {
        const response = await fetch(`${API_URL}/parties?per_page=500`, { headers });
        if (response.ok) {
...
async function loadTransports() {
    try {
        const response = await fetch(`${API_URL}/transports?per_page=500`, { headers });
        if (response.ok) {
            const data = await response.json();
...
function loadAvailableStocks(forceRefresh = false) {
    ...
    stocksPromise = (async () => {
        try {
            const response = await fetch(`${API_URL}/stocks?per_page=500`, { headers });
            if (response.ok) {
                const data = await response.json();
